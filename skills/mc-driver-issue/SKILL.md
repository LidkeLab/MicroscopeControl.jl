...
| A DAQ-backed light (`CrystaLaser`, `VortranLaser`, `DaqTrLight`) constructs with a `@warn "No NI-DAQ devices found"` or "Failed to initialize NI-DAQ", and every later call fails | These devices are **not** serial. They drive NI-DAQ AO/DO channels through an `NIdaq` built in their constructor, which picks `devs[1]` (Crysta, Vortran) or `devs[device_index]` (DaqTrLight, default 2). Wrong device index, missing NI-DAQmx runtime, or the card not enumerated. | `NIDAQcard.showdevices(NIdaq())` from Julia; compare with NI MAX. Fix the index or the runtime, not the driver. |
