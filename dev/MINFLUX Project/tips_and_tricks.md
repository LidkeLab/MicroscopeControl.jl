...
As of July 2025 the microscope is in a state of development where it can be tricky to use. Here is what we've learned in our summer of building and operating it.
...
The 3D printed stage is not exactly level. Moving the stage a lot can cause the sample to drift in and out of focus. There is another 3D printed stage that is more sturdy located next to the monitor of the RAMAN desktop, however, it lacks the ability to move closer to the objective, which may pose problems.
...
### **Problem:** Freezing after `[ Info: Triggerscope version: SD Settings Loaded` causing `ERROR: Failed to read response from scope: Resetting port` 

**Solution:** `shutdown()` the triggerscope and try again until the function works. This is less of a solution and more of a horrible work around because I cannot find out where this is coming from.  This usually takes 3-4 attempts before it works. Unplugging and replugging the triggerscope seems to have no effect, and neither does turning the scope on and off. 